...
| document, PDF, form, assessment, POC, plan of care, CMS-485, kardex, welcome package, supervisory, form builder, document template, question type, itemType, adapter, answer mapping, question mapping, change chain, PatientDocumentContentItem, htmlTemplateId, sectionCode, checkbox distribution, cross-section pulling, addendum, answer transformation, POCItems, POC duties, duty list, getPocQuestionWithAnswer, formatFrequencyText, TaskWhenToPerform, zPOCItemsDocumentAnswer | `03-domains/CLINICAL_DOCUMENTS.md` |
...
| patient, profile, status, diagnosis, ICD, medication, allergy, staffing preference, emergency contact, certification period, start of care, discharge, service_type, HHA, PCA | `03-domains/PATIENTS.md` |
...
| HHAeXchange, HHAX, IVR, POC codes, rn_platform_poc_item_code, plan_of_care_item_code_pdf_enable, poc-code-mapping, POC code mapping | `03-domains/CLINICAL_DOCUMENTS.md` |
...
| **CLINICAL_DOCUMENTS.md** | All document types (PA, POC, CMS-485, Kardex, Welcome Package, Supervisory). Form builder, 30 question types, full PatientDocumentContentItem interface (~60 fields), adapter pipeline (answer transformation for PDF), 5 mapping types (checkbox distribution, multi-question aggregation, value mapping, cross-section pulling, parent answer mapping), 11-area change chain for question modifications, 3 PDF pipelines, CMS-485 cross-document mapping, nursing database questions (61 DatabaseLinkType values), agency branding. **Two parallel POC systems**: Legacy (`plan_of_care_item` table) vs RN Platform (TypeScript constants + AI rules, versioned v1-v7). POC question type (`itemType: "POC"` with `POCItems` array), answer schema (`zPOCItemsDocumentAnswer` with `TaskWhenToPerform`), full POC pipeline, IVR/HHA Exchange code mapping, POC-to-supervisory mapping. |
...
| **PATIENTS.md** | Patient entity — 6 profile tabs, 12 statuses, per-patient contracts, certification periods, diagnosis codes (ICD-10), medication profiles, staffing preferences, emergency contacts, multiple ID systems, `service_type` (HHA/PCA/Other — set at broadcast Step 3). |
