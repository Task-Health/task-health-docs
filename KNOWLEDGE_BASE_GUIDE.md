...
| **Clinical Documents** | `CLINICAL_DOCUMENTS.md` | The entire document system — Patient Assessment (v10, 8 pages), POC (v7, 42 duties), CMS-485 (28 fields), Emergency Kardex, Welcome Package, Aide Supervisory. Form builder, full PatientDocumentContentItem interface (~60 fields), 30 question types, adapter pipeline (answer transformation for PDFs), 5 mapping types, 11-area change chain checklist, 3 PDF generation pipelines, CMS-485 cross-document mapping, cross-document nursing database questions (61 DatabaseLinkType values), document versioning, agency branding. Two parallel POC systems (Legacy DB-driven vs RN Platform TypeScript-driven). POC question type (`itemType: "POC"` with `POCItems`), answer schema (`zPOCItemsDocumentAnswer` with `TaskWhenToPerform`), full POC pipeline, IVR/HHA Exchange code mapping, POC-to-supervisory mapping. |
...
| **Patients** | `PATIENTS.md` | Patient entity — 6 profile tabs, 12 statuses, per-patient contracts, certification periods, diagnosis codes (ICD-10), medication profiles, staffing preferences, emergency contacts, multiple ID systems, `service_type` (HHA/PCA/Other — set at broadcast Step 3). |
...
| FULL_SCHEMA.md | ✅ Substantial | ~600+ lines, ~60 core tables across 12 sections. 837-table catalog at bottom. **Added:** Corrected `plan_of_care_item` columns (`planOfCareTypeId`, `documentItemType`, `is_personal_care`, `is_active`), audit columns on `rn_platform_poc_item_code`, `service_type` on patient table. |
| ENTITY_RELATIONSHIPS.md | ✅ Substantial | ~400+ lines. Full ERD, 9 junction tables, 8 relationship patterns, FK index reference. **Updated:** Pattern 7 rewritten to document two parallel POC systems (Legacy DB-driven vs RN Platform TypeScript-driven), including answer schema and IVR code flow. |
| TASK_LIFECYCLE.md | ✅ Substantial | ~692 lines |
| CLINICAL_DOCUMENTS.md | ✅ Substantial | ~1,500+ lines. Full PatientDocumentContentItem interface (~60 fields), 30 question types, adapter pipeline, 5 answer mapping types, 11-area change chain, 3 PDF pipelines, CMS-485 cross-doc mapping, mobile API endpoint. **Added:** Two parallel POC systems (Legacy vs RN Platform), POC question type (`itemType: "POC"` with `POCItems`), answer schema (`zPOCItemsDocumentAnswer` with `TaskWhenToPerform`), full POC pipeline, IVR/HHA Exchange code mapping, version management, POC-to-supervisory mapping. |
| AI_SYSTEM.md | ✅ Substantial | ~800+ lines. 36 generation rules, 20 review rules with actual prompt content, full POC rules engine, version-specific AI rules section. **Added:** Corrected POC answer format with `TaskWhenToPerform` union type, answer processing in `PatientDocumentAITextGenerationCtrl.ts`, updated `prepareInputs()` description noting `service_type` is NOT currently passed. |
...
| PATIENTS.md | ✅ Created | ~260+ lines. 6 profile tabs, 12 statuses, cert periods, diagnosis codes, schema. **Added:** `service_type` column (HHA/PCA/Other, set at broadcast Step 3 from agency portal `ServiceTypeField.tsx`). |
| CONTRACTS_AND_AGENCIES.md | ✅ Created | ~200+ lines. Setup fields, billing/rounding, EDI, issue types, template overrides. **Updated:** Section 6 expanded with full IVR/HHA Exchange code mapping details — DB tables, default codes, agency overrides, PDF rendering flow. |
